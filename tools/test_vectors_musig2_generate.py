 ...
 *
 * The BIP-MuSig2 KeySort vectors are tested by test_sort_vectors in
 * src/tests.c. */
"""
    % sys.argv[0]
)
...
with open(sys.argv[1] + "/sign_verify_vectors.json", "r") as f:
    ...
    s += (
        """
/* Valid signing cases omit explicit nonce indices; these tests use the first
 * secnonce/pubnonce from the vector for the signer under test. */
