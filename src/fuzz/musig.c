...
enum {
    ...
    SECP256K1_FUZZ_MUSIG_FORCE_NONCECOEF_ONE = 4,
    SECP256K1_FUZZ_MUSIG_FORCE_ZERO_FIRST_SCALAR = 5
};
...
static size_t secp256k1_fuzz_musig_force_first_zero_sha256_matches = 0;
...
static int secp256k1_fuzz_musig_force_noncecoef_one_active = 0;
static size_t secp256k1_fuzz_musig_force_noncecoef_one_matches = 0;
...
static void secp256k1_fuzz_musig_sha256_compression(uint32_t *state, const unsigned char *blocks64, size_t n_blocks) {
    int first_scalar_padding = 0;
    int second_scalar_padding = 0;
    int keyagg_coefficient_padding = 0;
    int noncecoef_midstate = 0;
    int noncecoef_padding = 0;
    size_t i;
    ...
    if (secp256k1_fuzz_musig_force_zero_sha256_state == SECP256K1_FUZZ_MUSIG_FORCE_ZERO_FIRST_SCALAR) {
        /* In the current MuSig nonce transcript, byte 48 is the loop index
         * and byte 49 starts the first final padding block. */
        first_scalar_padding = n_blocks == 1 && blocks64[48] == 0 && blocks64[49] == 0x80
            && blocks64[62] == 0x07 && blocks64[63] == 0x88;
        for (i = 50; first_scalar_padding && i < 62; i++) {
            first_scalar_padding &= blocks64[i] == 0;
        }
        if (first_scalar_padding) {
            memset(state, 0, 8 * sizeof(*state));
            secp256k1_fuzz_musig_force_first_zero_sha256_matches++;
        }
    } else if (secp256k1_fuzz_musig_force_zero_sha256_state == SECP256K1_FUZZ_MUSIG_FORCE_ZERO_SECOND_SCALAR) {
        if (secp256k1_fuzz_musig_force_second_zero_sha256_pending) {
            memset(state, 0, 8 * sizeof(*state));
            secp256k1_fuzz_musig_force_second_zero_sha256_pending = 0;
        }
        /* In the current MuSig nonce transcript, byte 48 is the loop index
         * and byte 49 starts the first final padding block. */
        second_scalar_padding = n_blocks == 1 && blocks64[48] == 1 && blocks64[49] == 0x80
            && blocks64[62] == 0x07 && blocks64[63] == 0x88;
        for (i = 50; second_scalar_padding && i < 62; i++) {
            second_scalar_padding &= blocks64[i] == 0;
        }
        if (second_scalar_padding) {
            memset(state, 0, 8 * sizeof(*state));
            secp256k1_fuzz_musig_force_second_zero_sha256_pending = 1;
            secp256k1_fuzz_musig_force_second_zero_sha256_matches++;
        }
    ...
    } else if (secp256k1_fuzz_musig_force_zero_sha256_state == SECP256K1_FUZZ_MUSIG_FORCE_NONCECOEF_ONE) {
        if (noncecoef_midstate) {
            /* The nonce-coefficient transcript starts after the 64-byte
             * tagged-hash prefix and is 130 bytes long. Its final length block
             * therefore ends in the 64-bit bit count 0x610. */
            secp256k1_fuzz_musig_force_noncecoef_one_active = 1;
        }
        noncecoef_padding = secp256k1_fuzz_musig_force_noncecoef_one_active && n_blocks == 1
            && blocks64[56] == 0 && blocks64[57] == 0 && blocks64[58] == 0 && blocks64[59] == 0
            && blocks64[60] == 0 && blocks64[61] == 0 && blocks64[62] == 0x06 && blocks64[63] == 0x10;
        if (noncecoef_padding) {
            memcpy(state, secp256k1_fuzz_musig_scalar_one_state, sizeof(secp256k1_fuzz_musig_scalar_one_state));
            secp256k1_fuzz_musig_force_noncecoef_one_active = 0;
            secp256k1_fuzz_musig_force_noncecoef_one_matches++;
        }
    }
}
...
static void secp256k1_fuzz_check_musig_zero_noncecoef(secp256k1_context *ctx, const unsigned char *msg32, const secp256k1_musig_keyagg_cache *keyagg_cache) {
    static const unsigned char scalar_two[32] = {
    ...
    unsigned char zero32[32] = { 0 };
    secp256k1_pubkey generator;
    secp256k1_pubkey second;
    secp256k1_musig_aggnonce aggnonce;
    secp256k1_musig_session session;
    size_t serialized_len;
    size_t noncecoef_calls;
    size_t challenge_calls;

    /* With b == 0, the effective nonce is R1 regardless of R2. Using 2G for
     * R2 makes accidentally evaluating the second component observable. */
    FUZZ_CHECK(secp256k1_ec_pubkey_create(ctx, &generator, secp256k1_fuzz_scalar_one) == 1);
    second = generator;
    FUZZ_CHECK(secp256k1_ec_pubkey_tweak_mul(ctx, &second, scalar_two) == 1);
    serialized_len = 33;
    FUZZ_CHECK(secp256k1_ec_pubkey_serialize(ctx, aggnonce66, &serialized_len, &generator, SECP256K1_EC_COMPRESSED) == 1);
    FUZZ_CHECK(serialized_len == 33);
    serialized_len = 33;
    FUZZ_CHECK(secp256k1_ec_pubkey_serialize(ctx, aggnonce66 + 33, &serialized_len, &second, SECP256K1_EC_COMPRESSED) == 1);
    FUZZ_CHECK(serialized_len == 33);
    FUZZ_CHECK(secp256k1_musig_aggnonce_parse(ctx, &aggnonce, aggnonce66) == 1);
    ...
    FUZZ_CHECK(secp256k1_musig_nonce_process(ctx, &session, &aggnonce, msg32, keyagg_cache) == 1);
    noncecoef_calls = secp256k1_fuzz_musig_noncecoef_sha256_compression_calls;
    challenge_calls = secp256k1_fuzz_musig_challenge_sha256_compression_calls;
    secp256k1_fuzz_musig_force_zero_sha256_state = 0;
    secp256k1_context_set_sha256_compression(ctx, NULL);

    FUZZ_CHECK(noncecoef_calls != 0);
    FUZZ_CHECK(challenge_calls != 0);
    FUZZ_CHECK(memcmp(session.data + 37, zero32, sizeof(zero32)) == 0);
    serialized_len = 33;
    FUZZ_CHECK(secp256k1_ec_pubkey_serialize(ctx, expected33, &serialized_len, &generator, SECP256K1_EC_COMPRESSED) == 1);
    FUZZ_CHECK(serialized_len == 33);
    FUZZ_CHECK(session.data[4] == (unsigned char)(expected33[0] == 0x03));
    FUZZ_CHECK(memcmp(session.data + 5, expected33 + 1, 32) == 0);
}

static void secp256k1_fuzz_check_musig_finite_nonce_cancellation(secp256k1_context *ctx, const unsigned char *msg32, const secp256k1_musig_keyagg_cache *keyagg_cache) {
    unsigned char aggnonce66[66];
    unsigned char expected33[33];
    secp256k1_pubkey generator;
    secp256k1_pubkey inverse;
    secp256k1_musig_aggnonce aggnonce;
    secp256k1_musig_session session;
    size_t serialized_len;
    size_t noncecoef_calls;
    ...
    /* Finite G + (-G) reaches the BIP327 fallback only after the group
     * addition, unlike the existing aggregate-infinity identity fixture. */
    FUZZ_CHECK(secp256k1_ec_pubkey_create(ctx, &generator, secp256k1_fuzz_scalar_one) == 1);
    inverse = generator;
    FUZZ_CHECK(secp256k1_ec_pubkey_negate(ctx, &inverse) == 1);
    serialized_len = 33;
    FUZZ_CHECK(secp256k1_ec_pubkey_serialize(ctx, aggnonce66, &serialized_len, &generator, SECP256K1_EC_COMPRESSED) == 1);
    FUZZ_CHECK(serialized_len == 33);
    serialized_len = 33;
    FUZZ_CHECK(secp256k1_ec_pubkey_serialize(ctx, aggnonce66 + 33, &serialized_len, &inverse, SECP256K1_EC_COMPRESSED) == 1);
    FUZZ_CHECK(serialized_len == 33);
    FUZZ_CHECK(secp256k1_musig_aggnonce_parse(ctx, &aggnonce, aggnonce66) == 1);

    secp256k1_fuzz_musig_force_zero_sha256_state = 0;
    secp256k1_fuzz_musig_force_noncecoef_one_active = 0;
    secp256k1_fuzz_musig_force_noncecoef_one_matches = 0;
    secp256k1_fuzz_musig_noncecoef_sha256_compression_calls = 0;
    secp256k1_fuzz_musig_challenge_sha256_compression_calls = 0;
    secp256k1_context_set_sha256_compression(ctx, secp256k1_fuzz_musig_sha256_compression);
    secp256k1_fuzz_musig_force_zero_sha256_state = SECP256K1_FUZZ_MUSIG_FORCE_NONCECOEF_ONE;
    FUZZ_CHECK(secp256k1_musig_nonce_process(ctx, &session, &aggnonce, msg32, keyagg_cache) == 1);
    noncecoef_calls = secp256k1_fuzz_musig_noncecoef_sha256_compression_calls;
    challenge_calls = secp256k1_fuzz_musig_challenge_sha256_compression_calls;
    secp256k1_fuzz_musig_force_zero_sha256_state = 0;
    ...
    FUZZ_CHECK(noncecoef_calls != 0);
    FUZZ_CHECK(secp256k1_fuzz_musig_force_noncecoef_one_matches == 1);
    FUZZ_CHECK(challenge_calls != 0);
    FUZZ_CHECK(memcmp(session.data + 37, secp256k1_fuzz_scalar_one, 32) == 0);
    serialized_len = 33;
    FUZZ_CHECK(secp256k1_ec_pubkey_serialize(ctx, expected33, &serialized_len, &generator, SECP256K1_EC_COMPRESSED) == 1);
    FUZZ_CHECK(serialized_len == 33);
    FUZZ_CHECK(session.data[4] == (unsigned char)(expected33[0] == 0x03));
...
static void secp256k1_fuzz_check_musig_static_pubnonce_codec(const secp256k1_context *ctx, const unsigned char *input66) {
    unsigned char normal_serialized[66];
    unsigned char static_serialized[66];
    unsigned char zero_nonce[sizeof(secp256k1_musig_pubnonce)] = { 0 };
    secp256k1_musig_pubnonce normal_nonce;
    secp256k1_musig_pubnonce static_nonce;
    secp256k1_musig_pubnonce static_reparsed;
    secp256k1_musig_pubnonce nonce_before;
    int normal_ret;
    int static_ret;

    memset(&normal_nonce, 0xA5, sizeof(normal_nonce));
    memset(&static_nonce, 0x5A, sizeof(static_nonce));
    normal_ret = secp256k1_musig_pubnonce_parse(ctx, &normal_nonce, input66);
    static_ret = secp256k1_musig_pubnonce_parse(secp256k1_context_static, &static_nonce, input66);
    FUZZ_CHECK(static_ret == normal_ret);
    if (normal_ret) {
        nonce_before = normal_nonce;
        FUZZ_CHECK(secp256k1_musig_pubnonce_serialize(ctx, normal_serialized, &normal_nonce) == 1);
        FUZZ_CHECK(memcmp(&normal_nonce, &nonce_before, sizeof(normal_nonce)) == 0);
        nonce_before = static_nonce;
        FUZZ_CHECK(secp256k1_musig_pubnonce_serialize(secp256k1_context_static, static_serialized, &static_nonce) == 1);
        FUZZ_CHECK(memcmp(&static_nonce, &nonce_before, sizeof(static_nonce)) == 0);
        FUZZ_CHECK(memcmp(static_serialized, normal_serialized, sizeof(static_serialized)) == 0);
        FUZZ_CHECK(secp256k1_musig_pubnonce_parse(secp256k1_context_static, &static_reparsed, static_serialized) == 1);
        FUZZ_CHECK(secp256k1_musig_pubnonce_serialize(secp256k1_context_static, static_serialized, &static_reparsed) == 1);
        FUZZ_CHECK(memcmp(static_serialized, normal_serialized, sizeof(static_serialized)) == 0);
    } else {
        FUZZ_CHECK(memcmp(&normal_nonce, zero_nonce, sizeof(normal_nonce)) == 0);
        FUZZ_CHECK(memcmp(&static_nonce, zero_nonce, sizeof(static_nonce)) == 0);
    }
}

static void secp256k1_fuzz_check_musig_static_aggnonce_codec(const secp256k1_context *ctx, const unsigned char *input66) {
    unsigned char normal_serialized[66];
    unsigned char static_serialized[66];
    unsigned char zero_nonce[sizeof(secp256k1_musig_aggnonce)] = { 0 };
    secp256k1_musig_aggnonce normal_nonce;
    secp256k1_musig_aggnonce static_nonce;
    secp256k1_musig_aggnonce static_reparsed;
    secp256k1_musig_aggnonce nonce_before;
    int normal_ret;
    int static_ret;

    memset(&normal_nonce, 0xA5, sizeof(normal_nonce));
    memset(&static_nonce, 0x5A, sizeof(static_nonce));
    normal_ret = secp256k1_musig_aggnonce_parse(ctx, &normal_nonce, input66);
    static_ret = secp256k1_musig_aggnonce_parse(secp256k1_context_static, &static_nonce, input66);
    FUZZ_CHECK(static_ret == normal_ret);
    if (normal_ret) {
        nonce_before = normal_nonce;
        FUZZ_CHECK(secp256k1_musig_aggnonce_serialize(ctx, normal_serialized, &normal_nonce) == 1);
        FUZZ_CHECK(memcmp(&normal_nonce, &nonce_before, sizeof(normal_nonce)) == 0);
        nonce_before = static_nonce;
        FUZZ_CHECK(secp256k1_musig_aggnonce_serialize(secp256k1_context_static, static_serialized, &static_nonce) == 1);
        FUZZ_CHECK(memcmp(&static_nonce, &nonce_before, sizeof(static_nonce)) == 0);
        FUZZ_CHECK(memcmp(static_serialized, normal_serialized, sizeof(static_serialized)) == 0);
        FUZZ_CHECK(secp256k1_musig_aggnonce_parse(secp256k1_context_static, &static_reparsed, static_serialized) == 1);
        FUZZ_CHECK(secp256k1_musig_aggnonce_serialize(secp256k1_context_static, static_serialized, &static_reparsed) == 1);
        FUZZ_CHECK(memcmp(static_serialized, normal_serialized, sizeof(static_serialized)) == 0);
    } else {
        FUZZ_CHECK(memcmp(&normal_nonce, zero_nonce, sizeof(normal_nonce)) == 0);
        FUZZ_CHECK(memcmp(&static_nonce, zero_nonce, sizeof(static_nonce)) == 0);
...
static void secp256k1_fuzz_check_musig_static_partial_sig_codec(const secp256k1_context *ctx, const unsigned char *input32) {
    unsigned char normal_serialized[32];
    unsigned char static_serialized[32];
    unsigned char zero_sig[sizeof(secp256k1_musig_partial_sig)] = { 0 };
    secp256k1_musig_partial_sig normal_sig;
    secp256k1_musig_partial_sig static_sig;
    secp256k1_musig_partial_sig static_reparsed;
    secp256k1_musig_partial_sig sig_before;
    int normal_ret;
    int static_ret;

    memset(&normal_sig, 0xA5, sizeof(normal_sig));
    memset(&static_sig, 0x5A, sizeof(static_sig));
    normal_ret = secp256k1_musig_partial_sig_parse(ctx, &normal_sig, input32);
    static_ret = secp256k1_musig_partial_sig_parse(secp256k1_context_static, &static_sig, input32);
    FUZZ_CHECK(static_ret == normal_ret);
    if (normal_ret) {
        sig_before = normal_sig;
        FUZZ_CHECK(secp256k1_musig_partial_sig_serialize(ctx, normal_serialized, &normal_sig) == 1);
        FUZZ_CHECK(memcmp(&normal_sig, &sig_before, sizeof(normal_sig)) == 0);
        sig_before = static_sig;
        FUZZ_CHECK(secp256k1_musig_partial_sig_serialize(secp256k1_context_static, static_serialized, &static_sig) == 1);
        FUZZ_CHECK(memcmp(&static_sig, &sig_before, sizeof(static_sig)) == 0);
        FUZZ_CHECK(memcmp(static_serialized, normal_serialized, sizeof(static_serialized)) == 0);
        FUZZ_CHECK(secp256k1_musig_partial_sig_parse(secp256k1_context_static, &static_reparsed, static_serialized) == 1);
        FUZZ_CHECK(secp256k1_musig_partial_sig_serialize(secp256k1_context_static, static_serialized, &static_reparsed) == 1);
        FUZZ_CHECK(memcmp(static_serialized, normal_serialized, sizeof(static_serialized)) == 0);
    } else {
...
static void secp256k1_fuzz_check_musig_static_public_codecs(
    const secp256k1_context *ctx,
    const unsigned char *valid_pubnonce66,
    const unsigned char *invalid_pubnonce66,
    const unsigned char *valid_aggnonce66,
    const unsigned char *zero_aggnonce66,
    const unsigned char *invalid_aggnonce66
) {
    secp256k1_fuzz_check_musig_static_pubnonce_codec(ctx, valid_pubnonce66);
    secp256k1_fuzz_check_musig_static_pubnonce_codec(ctx, invalid_pubnonce66);
    secp256k1_fuzz_check_musig_static_aggnonce_codec(ctx, valid_aggnonce66);
    secp256k1_fuzz_check_musig_static_aggnonce_codec(ctx, zero_aggnonce66);
    secp256k1_fuzz_check_musig_static_aggnonce_codec(ctx, invalid_aggnonce66);
    secp256k1_fuzz_check_musig_static_partial_sig_codec(ctx, secp256k1_fuzz_scalar_zero);
    secp256k1_fuzz_check_musig_static_partial_sig_codec(ctx, secp256k1_fuzz_scalar_order_minus_one);
    secp256k1_fuzz_check_musig_static_partial_sig_codec(ctx, secp256k1_fuzz_scalar_order);
}
...
static void secp256k1_fuzz_check_musig_nonce_gen_session_random_cache_alias(const secp256k1_context *ctx, const unsigned char *seckey32, const secp256k1_pubkey *pubkey, const unsigned char *msg32, const secp256k1_musig_keyagg_cache *keyagg_cache, const unsigned char *extra_input32) {
    unsigned char session_rand_before[32];
    unsigned char expected_k64[64];
    unsigned char expected_pubnonce66[66];
    unsigned char serialized_pubnonce66[66];
    ...
    unsigned char zero132[132] = { 0 };
    secp256k1_musig_keyagg_cache cache_before;
    secp256k1_musig_keyagg_cache cache_alias;
    secp256k1_musig_secnonce secnonce;
    secp256k1_musig_pubnonce pubnonce;
    int expected_ret;
    int actual_ret;

    /* The session-random buffer is explicitly invalidated on success. Keep a
     * second cache for the independent transcript because the actual call
     * deliberately exercises clearing the first 32 bytes of its cache alias. */
    cache_alias = *keyagg_cache;
    cache_before = cache_alias;
    memcpy(session_rand_before, cache_before.data, sizeof(session_rand_before));
    FUZZ_CHECK(memcmp(session_rand_before, zero32, sizeof(session_rand_before)) != 0);
    expected_ret = secp256k1_fuzz_musig_nonce_reference(ctx, expected_k64, expected_pubnonce66, session_rand_before, seckey32, pubkey, msg32, &cache_before, extra_input32);

    memset(&secnonce, 0xA5, sizeof(secnonce));
    memset(&pubnonce, 0x5A, sizeof(pubnonce));
    actual_ret = secp256k1_musig_nonce_gen(ctx, &secnonce, &pubnonce, cache_alias.data, seckey32, pubkey, msg32, &cache_alias, extra_input32);
    FUZZ_CHECK(actual_ret == expected_ret);
    if (expected_ret == 0) {
        FUZZ_CHECK(memcmp(&secnonce, zero132, sizeof(secnonce)) == 0);
        FUZZ_CHECK(memcmp(&pubnonce, zero132, sizeof(pubnonce)) == 0);
        FUZZ_CHECK(memcmp(&cache_alias, &cache_before, sizeof(cache_alias)) == 0);
        return;
    }
    FUZZ_CHECK(memcmp(cache_alias.data, zero32, sizeof(zero32)) == 0);
    FUZZ_CHECK(memcmp(cache_alias.data + sizeof(zero32), cache_before.data + sizeof(zero32), sizeof(cache_alias.data) - sizeof(zero32)) == 0);
    FUZZ_CHECK(memcmp(secnonce.data + 4, expected_k64, sizeof(expected_k64)) == 0);
    FUZZ_CHECK(secp256k1_musig_pubnonce_serialize(ctx, serialized_pubnonce66, &pubnonce) == 1);
    FUZZ_CHECK(memcmp(serialized_pubnonce66, expected_pubnonce66, sizeof(serialized_pubnonce66)) == 0);
}
...
static void secp256k1_fuzz_check_musig_static_public_signing(const secp256k1_musig_pubnonce *pubnonces, const secp256k1_pubkey *pubkeys, const secp256k1_musig_keyagg_cache *keyagg_cache, const secp256k1_musig_session *session, const secp256k1_musig_partial_sig * const *partial_sig_ptrs, size_t n_signers, const unsigned char *expected_sig64) {
    secp256k1_musig_pubnonce pubnonce_before[SECP256K1_FUZZ_MUSIG_MAX_SIGNERS];
    secp256k1_pubkey pubkey_before[SECP256K1_FUZZ_MUSIG_MAX_SIGNERS];
    secp256k1_musig_partial_sig partial_sig_before[SECP256K1_FUZZ_MUSIG_MAX_SIGNERS];
    secp256k1_musig_keyagg_cache cache_before = *keyagg_cache;
    secp256k1_musig_session session_before = *session;
    unsigned char static_sig64[64];
    size_t i;

    FUZZ_CHECK(n_signers <= SECP256K1_FUZZ_MUSIG_MAX_SIGNERS);
    for (i = 0; i < n_signers; i++) {
        pubnonce_before[i] = pubnonces[i];
        pubkey_before[i] = pubkeys[i];
        partial_sig_before[i] = *partial_sig_ptrs[i];
        FUZZ_CHECK(secp256k1_musig_partial_sig_verify(secp256k1_context_static, partial_sig_ptrs[i], &pubnonces[i], &pubkeys[i], keyagg_cache, session) == 1);
    }
    memset(static_sig64, 0xA5, sizeof(static_sig64));
    FUZZ_CHECK(secp256k1_musig_partial_sig_agg(secp256k1_context_static, static_sig64, session, partial_sig_ptrs, n_signers) == 1);
    FUZZ_CHECK(memcmp(static_sig64, expected_sig64, sizeof(static_sig64)) == 0);

    FUZZ_CHECK(memcmp(keyagg_cache, &cache_before, sizeof(cache_before)) == 0);
    FUZZ_CHECK(memcmp(session, &session_before, sizeof(session_before)) == 0);
    for (i = 0; i < n_signers; i++) {
        FUZZ_CHECK(memcmp(&pubnonces[i], &pubnonce_before[i], sizeof(pubnonce_before[i])) == 0);
        FUZZ_CHECK(memcmp(&pubkeys[i], &pubkey_before[i], sizeof(pubkey_before[i])) == 0);
        FUZZ_CHECK(memcmp(partial_sig_ptrs[i], &partial_sig_before[i], sizeof(partial_sig_before[i])) == 0);
    }
}
...
static void secp256k1_fuzz_check_musig_sign_roundtrip(secp256k1_context *ctx, const unsigned char *input, size_t size, const unsigned char (*seckey)[32], const secp256k1_keypair *keypairs, const secp256k1_pubkey *pubkeys, size_t n_pubkeys, const unsigned char *msg32) {
    ...
    if (size == sizeof("MuSig static public signing\n") - 1
            && memcmp(input, "MuSig static public signing\n", sizeof("MuSig static public signing\n") - 1) == 0) {
        secp256k1_fuzz_check_musig_static_public_signing(pubnonce, pubkeys, &keyagg_cache, &session, partial_sig_ptrs, n_signers, sig64);
    }
    secp256k1_fuzz_check_musig_final_sig_equation(ctx, sig64, &session, &agg_pk, msg32);
    FUZZ_CHECK(secp256k1_schnorrsig_verify(ctx, sig64, msg32, 32, &agg_pk) == 1);
...
static void secp256k1_fuzz_check_musig_all_identical_sign_roundtrip(secp256k1_context *ctx, const unsigned char *input, size_t size) {
    enum { N_IDENTICAL_SIGNERS = 10 };
    static const unsigned char trigger[] = "all-identical MuSig signing\n";
    unsigned char seckey[N_IDENTICAL_SIGNERS][32] = { { 0 } };
    unsigned char msg32[32];
    secp256k1_keypair keypairs[N_IDENTICAL_SIGNERS];
    secp256k1_pubkey pubkeys[N_IDENTICAL_SIGNERS];
    size_t i;
    ...
    secp256k1_fuzz_derive(msg32, sizeof(msg32), input, size, 491);
    for (i = 0; i < N_IDENTICAL_SIGNERS; i++) {
        seckey[i][31] = 1;
        FUZZ_CHECK(secp256k1_keypair_create(ctx, &keypairs[i], seckey[i]) == 1);
        FUZZ_CHECK(secp256k1_ec_pubkey_create(ctx, &pubkeys[i], seckey[i]) == 1);
    }
    secp256k1_fuzz_check_musig_sign_roundtrip(ctx, input, size, (const unsigned char (*)[32])seckey,
        keypairs, pubkeys, N_IDENTICAL_SIGNERS, msg32);
    memset(seckey, 0, sizeof(seckey));
    memset(keypairs, 0, sizeof(keypairs));
}
...
static void secp256k1_fuzz_check_musig_nonce_gen_first_zero_scalar_failure(secp256k1_context *ctx, const secp256k1_pubkey *pubkey, const unsigned char *seckey, const unsigned char *msg32, const secp256k1_musig_keyagg_cache *keyagg_cache, const unsigned char *extra_input32, const unsigned char *session_rand32) {
    unsigned char session_rand[32];
    unsigned char session_rand_before[32];
    unsigned char zero132[132] = { 0 };
    secp256k1_musig_secnonce secnonce;
    ...
    secp256k1_fuzz_musig_force_zero_sha256_state = 0;
    secp256k1_fuzz_musig_force_first_zero_sha256_matches = 0;
    secp256k1_context_set_sha256_compression(ctx, secp256k1_fuzz_musig_sha256_compression);
    secp256k1_fuzz_musig_sha256_compression_calls = 0;
    secp256k1_fuzz_musig_force_zero_sha256_state = SECP256K1_FUZZ_MUSIG_FORCE_ZERO_FIRST_SCALAR;
    FUZZ_CHECK(secp256k1_musig_nonce_gen(ctx, &secnonce, &pubnonce, session_rand, seckey, pubkey, msg32, keyagg_cache, extra_input32) == 0);
    secp256k1_fuzz_musig_force_zero_sha256_state = 0;
    secp256k1_context_set_sha256_compression(ctx, NULL);

    FUZZ_CHECK(secp256k1_fuzz_musig_sha256_compression_calls != 0);
    FUZZ_CHECK(secp256k1_fuzz_musig_force_first_zero_sha256_matches == 1);
    FUZZ_CHECK(memcmp(session_rand, session_rand_before, sizeof(session_rand)) == 0);
    FUZZ_CHECK(memcmp(secnonce.data, zero132, sizeof(secnonce.data)) == 0);
    FUZZ_CHECK(memcmp(pubnonce.data, zero132, sizeof(pubnonce.data)) == 0);
}

static void secp256k1_fuzz_check_musig_nonce_gen_second_zero_scalar_failure(secp256k1_context *ctx, const secp256k1_pubkey *pubkey, const unsigned char *seckey, const unsigned char *msg32, const secp256k1_musig_keyagg_cache *keyagg_cache, const unsigned char *extra_input32, const unsigned char *session_rand32) {
    unsigned char session_rand[32];
    unsigned char session_rand_before[32];
    unsigned char zero132[132] = { 0 };
    secp256k1_musig_secnonce secnonce;
    ...
    secp256k1_fuzz_musig_force_zero_sha256_state = 0;
    secp256k1_fuzz_musig_force_second_zero_sha256_pending = 0;
    secp256k1_fuzz_musig_force_second_zero_sha256_matches = 0;
    secp256k1_context_set_sha256_compression(ctx, secp256k1_fuzz_musig_sha256_compression);
    secp256k1_fuzz_musig_sha256_compression_calls = 0;
    secp256k1_fuzz_musig_force_zero_sha256_state = SECP256K1_FUZZ_MUSIG_FORCE_ZERO_SECOND_SCALAR;
    FUZZ_CHECK(secp256k1_musig_nonce_gen(ctx, &secnonce, &pubnonce, session_rand, seckey, pubkey, msg32, keyagg_cache, extra_input32) == 0);
    secp256k1_fuzz_musig_force_zero_sha256_state = 0;
    secp256k1_fuzz_musig_force_second_zero_sha256_pending = 0;
    secp256k1_context_set_sha256_compression(ctx, NULL);
    ...
    FUZZ_CHECK(secp256k1_fuzz_musig_sha256_compression_calls != 0);
    FUZZ_CHECK(secp256k1_fuzz_musig_force_second_zero_sha256_matches == 1);
    FUZZ_CHECK(memcmp(session_rand, session_rand_before, sizeof(session_rand)) == 0);
    FUZZ_CHECK(memcmp(secnonce.data, zero132, sizeof(secnonce.data)) == 0);
    FUZZ_CHECK(memcmp(pubnonce.data, zero132, sizeof(pubnonce.data)) == 0);
}
...
int LLVMFuzzerTestOneInput(const unsigned char *data, size_t size) {
    ...
    static const unsigned char first_zero_nonce_trigger[] = "MuSig nonce first zero scalar\n";
    static const unsigned char second_zero_nonce_trigger[] = "MuSig nonce second zero scalar\n";
    static const unsigned char zero_noncecoef_trigger[] = "MuSig zero nonce coefficient\n";
    static const unsigned char finite_nonce_cancellation_trigger[] = "MuSig finite nonce cancellation\n";
    ...
    if (size == sizeof("MuSig nonce session-random cache overlap\n") - 1
        && memcmp(input, "MuSig nonce session-random cache overlap\n", sizeof("MuSig nonce session-random cache overlap\n") - 1) == 0) {
        secp256k1_fuzz_check_musig_nonce_gen_session_random_cache_alias(ctx, seckey[0], &pubkeys[0], tweak, &cache, session_rand);
    }
    ...
    if (size == sizeof(zero_noncecoef_trigger) - 1
        && memcmp(input, zero_noncecoef_trigger, sizeof(zero_noncecoef_trigger) - 1) == 0) {
        secp256k1_fuzz_check_musig_zero_noncecoef(ctx, tweak, &single_cache);
    }
    if (size == sizeof(finite_nonce_cancellation_trigger) - 1
        && memcmp(input, finite_nonce_cancellation_trigger, sizeof(finite_nonce_cancellation_trigger) - 1) == 0) {
        secp256k1_fuzz_check_musig_finite_nonce_cancellation(ctx, tweak, &single_cache);
    }
    ...
    secp256k1_fuzz_check_musig_all_identical_sign_roundtrip(ctx, input, size);
    ...
    if (size == sizeof(first_zero_nonce_trigger) - 1
        && memcmp(input, first_zero_nonce_trigger, sizeof(first_zero_nonce_trigger) - 1) == 0) {
        secp256k1_fuzz_check_musig_nonce_gen_first_zero_scalar_failure(ctx, &pubkeys[0], seckey[0], tweak, &cache, tweak, session_rand);
    }
    if (size == sizeof(second_zero_nonce_trigger) - 1
        && memcmp(input, second_zero_nonce_trigger, sizeof(second_zero_nonce_trigger) - 1) == 0) {
        secp256k1_fuzz_check_musig_nonce_gen_second_zero_scalar_failure(ctx, &pubkeys[0], seckey[0], tweak, &cache, tweak, session_rand);
    }
    ...
    if (size == sizeof("MuSig static public codecs\n") - 1
            && memcmp(input, "MuSig static public codecs\n", sizeof("MuSig static public codecs\n") - 1) == 0) {
        secp256k1_fuzz_check_musig_static_public_codecs(
            ctx,
            fixed_pubnonce66,
            zero66,
            valid_aggnonce66,
