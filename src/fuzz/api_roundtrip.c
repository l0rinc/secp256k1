...
static void secp256k1_fuzz_check_ecdsa_verify_half_order(const secp256k1_context *ctx, const unsigned char *input, size_t size) {
    static const unsigned char trigger[] = "ecdsa verify half order\n";
    ...
    FUZZ_CHECK(secp256k1_fuzz_ecdsa_verify_reference(ctx, &sig, msg32, &pubkey) == 1);
...
/* Match Bitcoin Core's private BIP32 derivation boundary. The first vector is
 * hardened and hashes 0 || privkey; the second is non-hardened and hashes the
 * compressed parent key. HMAC-SHA512 is fixed from independent Core vectors,
 * so this helper checks the libsecp256k1 private-tweak boundary rather than
 * reimplementing Core's HMAC. */
static void secp256k1_fuzz_check_core_bip32_private_derivation(const secp256k1_context *ctx, const unsigned char *input, size_t inputlen) {
    static const unsigned char trigger[] = "core BIP32 private derivation composition\n";
    static const unsigned char parent_seckeys[2][32] = {
        {
    ...
    static const unsigned char expected_hmac[2][64] = {
        {
            0x04, 0xBF, 0xB2, 0xDD, 0x60, 0xFA, 0x89, 0x21,
            0xC2, 0xA4, 0x08, 0x5E, 0xC1, 0x55, 0x07, 0xA9,
            0x21, 0xF4, 0x9C, 0xDC, 0x83, 0x9F, 0x27, 0xF0,
            0xF2, 0x80, 0xE9, 0xC1, 0x49, 0x5D, 0x44, 0xB5,
    ...
    static const unsigned char expected_child_seckeys[2][32] = {
        {
            0xED, 0xB2, 0xE1, 0x4F, 0x9E, 0xE7, 0x7D, 0x26,
            0xDD, 0x93, 0xB4, 0xEC, 0xED, 0xE8, 0xD1, 0x6E,
            0xD4, 0x08, 0xCE, 0x14, 0x9B, 0x6C, 0xD8, 0x0B,
    ...
    for (i = 0; i < 2; i++) {
        unsigned char child_seckey[32];
        unsigned char static_child_seckey[32];
        ...
        size_t static_serialized_len = sizeof(static_serialized);

        FUZZ_CHECK(secp256k1_ec_seckey_verify(ctx, parent_seckeys[i]) == 1);
        FUZZ_CHECK(secp256k1_ec_seckey_verify(secp256k1_context_static, parent_seckeys[i]) == 1);
        memcpy(child_seckey, parent_seckeys[i], sizeof(child_seckey));
        memcpy(static_child_seckey, parent_seckeys[i], sizeof(static_child_seckey));
        FUZZ_CHECK(secp256k1_ec_seckey_tweak_add(ctx, child_seckey, expected_hmac[i]) == 1);
        FUZZ_CHECK(secp256k1_ec_seckey_tweak_add(secp256k1_context_static, static_child_seckey, expected_hmac[i]) == 1);
        FUZZ_CHECK(memcmp(child_seckey, expected_child_seckeys[i], sizeof(child_seckey)) == 0);
        FUZZ_CHECK(memcmp(static_child_seckey, child_seckey, sizeof(static_child_seckey)) == 0);

        FUZZ_CHECK(secp256k1_ec_pubkey_create(ctx, &child_pubkey, child_seckey) == 1);
        FUZZ_CHECK(secp256k1_ec_pubkey_serialize(ctx, serialized, &serialized_len, &child_pubkey, SECP256K1_EC_COMPRESSED) == 1);
        FUZZ_CHECK(secp256k1_ec_pubkey_serialize(secp256k1_context_static, static_serialized, &static_serialized_len, &child_pubkey, SECP256K1_EC_COMPRESSED) == 1);
        ...
        FUZZ_CHECK(memcmp(static_serialized, serialized, sizeof(static_serialized)) == 0);
    }
}

/* Match Bitcoin Core's default CKey::Sign sequence. The first two RFC6979
 * candidates have high R and are discarded by Core's low-R grinder; the third
 * candidate, with little-endian retry counter 2 as extra entropy, is the
 * serialized signature returned to the transaction signer. */
static void secp256k1_fuzz_check_core_ecdsa_sign_composition(const secp256k1_context *ctx, const unsigned char *input, size_t inputlen) {
    static const unsigned char trigger[] = "core ECDSA signing composition\n";
    ...
    static const unsigned char msg32[32] = {
        0xE3, 0x4E, 0x81, 0x2F, 0x4C, 0x65, 0x91, 0x56,
        0xAC, 0x22, 0x79, 0xB9, 0x2C, 0x22, 0xA5, 0x3C,
        0x98, 0x22, 0xAC, 0x10, 0x39, 0x6F, 0xE8, 0xDA,
        0x12, 0xA2, 0xFC, 0xFE, 0xF8, 0x81, 0x35, 0x66
    };
    static const unsigned char expected_pubkey[33] = {
        0x03, 0x0B, 0x4C, 0x86, 0x65, 0x85, 0xDD, 0x86,
        0x8A, 0x9D, 0x62, 0x34, 0x8A, 0x9C, 0xD0, 0x08,
        0xD6, 0xA3, 0x12, 0x93, 0x70, 0x48, 0xFF, 0xF3,
        0x16, 0x70, 0xE7, 0xE9, 0x20, 0xCF, 0xC7, 0xA7,
        0x44
    };
    static const unsigned char expected_compact[3][64] = {
        {
            0x8A, 0xE1, 0x48, 0xD1, 0x65, 0x7B, 0xFC, 0x50,
            0x9A, 0xC7, 0xE1, 0x18, 0xC5, 0xEA, 0xD6, 0x2D,
            0x4B, 0xB3, 0xEE, 0xD6, 0x08, 0xCC, 0xAD, 0x32,
            0x39, 0x59, 0xCF, 0xCF, 0x3C, 0xD7, 0x09, 0x33,
            0x14, 0xF8, 0xC7, 0xD8, 0x56, 0x38, 0x18, 0x1C,
            0xF7, 0xC0, 0x0A, 0xF0, 0xA0, 0x8E, 0x03, 0xE2,
            0x5A, 0x74, 0x77, 0x0D, 0x38, 0xE1, 0x2C, 0xD4,
            0xA7, 0x40, 0x66, 0x1A, 0x9B, 0x5C, 0x2F, 0xAA
        },
        {
            0xE7, 0xE8, 0x0E, 0xD0, 0xBA, 0xD5, 0x79, 0x55,
            0x07, 0x6A, 0x39, 0xEB, 0x54, 0xAB, 0xE7, 0x5B,
            0x37, 0x69, 0x9E, 0x13, 0x11, 0x5D, 0xD6, 0x47,
            0xA7, 0x56, 0x7F, 0xA4, 0xCF, 0x29, 0xA4, 0x13,
            0x3E, 0xBE, 0x84, 0x0D, 0xC6, 0x29, 0x94, 0xC4,
            0x68, 0x28, 0xCE, 0x79, 0x51, 0xCC, 0xFC, 0x3B,
            0x64, 0x58, 0xAD, 0xA2, 0x9A, 0xF0, 0x81, 0x60,
            0x4A, 0x22, 0x30, 0x9C, 0x06, 0x1A, 0x5A, 0xA4
        },
        {
            0x68, 0x66, 0x30, 0x52, 0xE6, 0xC2, 0x9C, 0x7E,
            0xD7, 0xAB, 0x02, 0xA6, 0x88, 0x52, 0x30, 0x15,
            0x08, 0x50, 0x3E, 0x79, 0x86, 0xB9, 0x75, 0x4E,
            0xC3, 0xE8, 0x68, 0x77, 0x2F, 0x2B, 0xF7, 0x39,
            0x28, 0xC6, 0xA3, 0x5B, 0x2E, 0x90, 0x25, 0x0D,
            0x31, 0x79, 0xF9, 0x6C, 0x2B, 0xB6, 0xB7, 0x72,
            0xE8, 0x89, 0xE9, 0xA1, 0x33, 0xA5, 0x15, 0x65,
            0x64, 0xA6, 0x96, 0x5A, 0x8C, 0xAA, 0x2B, 0x26
        }
    };
    static const unsigned char expected_der[3][72] = {
        {
            0x30, 0x45, 0x02, 0x21, 0x00, 0x8A, 0xE1, 0x48,
            0xD1, 0x65, 0x7B, 0xFC, 0x50, 0x9A, 0xC7, 0xE1,
            0x18, 0xC5, 0xEA, 0xD6, 0x2D, 0x4B, 0xB3, 0xEE,
            0xD6, 0x08, 0xCC, 0xAD, 0x32, 0x39, 0x59, 0xCF,
            0xCF, 0x3C, 0xD7, 0x09, 0x33, 0x02, 0x20, 0x14,
            0xF8, 0xC7, 0xD8, 0x56, 0x38, 0x18, 0x1C, 0xF7,
            0xC0, 0x0A, 0xF0, 0xA0, 0x8E, 0x03, 0xE2, 0x5A,
            0x74, 0x77, 0x0D, 0x38, 0xE1, 0x2C, 0xD4, 0xA7,
            0x40, 0x66, 0x1A, 0x9B, 0x5C, 0x2F, 0xAA
        },
        {
            0x30, 0x45, 0x02, 0x21, 0x00, 0xE7, 0xE8, 0x0E,
            0xD0, 0xBA, 0xD5, 0x79, 0x55, 0x07, 0x6A, 0x39,
            0xEB, 0x54, 0xAB, 0xE7, 0x5B, 0x37, 0x69, 0x9E,
            0x13, 0x11, 0x5D, 0xD6, 0x47, 0xA7, 0x56, 0x7F,
            0xA4, 0xCF, 0x29, 0xA4, 0x13, 0x02, 0x20, 0x3E,
            0xBE, 0x84, 0x0D, 0xC6, 0x29, 0x94, 0xC4, 0x68,
            0x28, 0xCE, 0x79, 0x51, 0xCC, 0xFC, 0x3B, 0x64,
            0x58, 0xAD, 0xA2, 0x9A, 0xF0, 0x81, 0x60, 0x4A,
            0x22, 0x30, 0x9C, 0x06, 0x1A, 0x5A, 0xA4
        },
        {
            0x30, 0x44, 0x02, 0x20, 0x68, 0x66, 0x30, 0x52,
            0xE6, 0xC2, 0x9C, 0x7E, 0xD7, 0xAB, 0x02, 0xA6,
            0x88, 0x52, 0x30, 0x15, 0x08, 0x50, 0x3E, 0x79,
            0x86, 0xB9, 0x75, 0x4E, 0xC3, 0xE8, 0x68, 0x77,
            0x2F, 0x2B, 0xF7, 0x39, 0x02, 0x20, 0x28, 0xC6,
            0xA3, 0x5B, 0x2E, 0x90, 0x25, 0x0D, 0x31, 0x79,
            0xF9, 0x6C, 0x2B, 0xB6, 0xB7, 0x72, 0xE8, 0x89,
            0xE9, 0xA1, 0x33, 0xA5, 0x15, 0x65, 0x64, 0xA6,
            0x96, 0x5A, 0x8C, 0xAA, 0x2B, 0x26
        }
    };
    static const size_t expected_der_len[3] = { 71, 71, 70 };
    secp256k1_pubkey pubkey;
    unsigned char serialized_pubkey[33];
    unsigned char static_serialized_pubkey[33];
    size_t serialized_pubkey_len = sizeof(serialized_pubkey);
    size_t static_serialized_pubkey_len = sizeof(static_serialized_pubkey);
    size_t i;
    ...
    FUZZ_CHECK(secp256k1_ec_seckey_verify(ctx, seckey) == 1);
    FUZZ_CHECK(secp256k1_ec_seckey_verify(secp256k1_context_static, seckey) == 1);
    FUZZ_CHECK(secp256k1_ec_pubkey_create(ctx, &pubkey, seckey) == 1);
    FUZZ_CHECK(secp256k1_ec_pubkey_serialize(ctx, serialized_pubkey, &serialized_pubkey_len, &pubkey, SECP256K1_EC_COMPRESSED) == 1);
    FUZZ_CHECK(secp256k1_ec_pubkey_serialize(secp256k1_context_static, static_serialized_pubkey, &static_serialized_pubkey_len, &pubkey, SECP256K1_EC_COMPRESSED) == 1);
    FUZZ_CHECK(serialized_pubkey_len == sizeof(expected_pubkey));
    FUZZ_CHECK(static_serialized_pubkey_len == serialized_pubkey_len);
    FUZZ_CHECK(memcmp(serialized_pubkey, expected_pubkey, sizeof(expected_pubkey)) == 0);
    FUZZ_CHECK(memcmp(static_serialized_pubkey, serialized_pubkey, sizeof(serialized_pubkey)) == 0);
    ...
    for (i = 0; i < 3; i++) {
        ...
        secp256k1_ecdsa_signature parsed_sig;
        size_t der_len = sizeof(der);
        size_t static_der_len = sizeof(static_der);

        if (i != 0) {
            extra[0] = (unsigned char)i;
        }
        FUZZ_CHECK(secp256k1_ecdsa_sign(ctx, &sig, msg32, seckey, secp256k1_nonce_function_rfc6979, i == 0 ? NULL : extra) == 1);
        FUZZ_CHECK(secp256k1_ecdsa_signature_serialize_compact(ctx, compact, &sig) == 1);
        FUZZ_CHECK(secp256k1_ecdsa_signature_serialize_compact(secp256k1_context_static, static_compact, &sig) == 1);
        FUZZ_CHECK(memcmp(compact, expected_compact[i], sizeof(compact)) == 0);
        FUZZ_CHECK(memcmp(static_compact, compact, sizeof(static_compact)) == 0);
        FUZZ_CHECK((compact[0] < 0x80) == (i == 2));
        FUZZ_CHECK(secp256k1_ecdsa_signature_serialize_der(ctx, der, &der_len, &sig) == 1);
        FUZZ_CHECK(secp256k1_ecdsa_signature_serialize_der(secp256k1_context_static, static_der, &static_der_len, &sig) == 1);
        FUZZ_CHECK(der_len == expected_der_len[i]);
        FUZZ_CHECK(static_der_len == der_len);
        FUZZ_CHECK(memcmp(der, expected_der[i], der_len) == 0);
        FUZZ_CHECK(memcmp(static_der, der, der_len) == 0);
        FUZZ_CHECK(secp256k1_ecdsa_signature_parse_der(ctx, &parsed_sig, der, der_len) == 1);
        FUZZ_CHECK(secp256k1_ecdsa_verify(secp256k1_context_static, &parsed_sig, msg32, &pubkey) == 1);
    }
}
...
int LLVMFuzzerTestOneInput(const unsigned char *data, size_t size) {
    ...
    secp256k1_ecdsa_signature explicit_sig;
    ...
    unsigned char sig_explicit64[64];
    ...
    FUZZ_CHECK(secp256k1_ecdsa_signature_serialize_compact(ctx, sig64, &sig) == 1);
    secp256k1_fuzz_ecdsa_sha256_compression_calls = 0;
    FUZZ_CHECK(secp256k1_ecdsa_sign(ctx, &explicit_sig, msg32, seckey, secp256k1_nonce_function_rfc6979, NULL) == 1);
    FUZZ_CHECK(secp256k1_fuzz_ecdsa_sha256_compression_calls != 0);
    FUZZ_CHECK(secp256k1_ecdsa_signature_serialize_compact(ctx, sig_explicit64, &explicit_sig) == 1);
    FUZZ_CHECK(memcmp(sig64, sig_explicit64, sizeof(sig64)) == 0);
    secp256k1_fuzz_ecdsa_sha256_compression_calls = 0;
    FUZZ_CHECK(secp256k1_ecdsa_sign(ctx, &explicit_sig, msg32, seckey, secp256k1_nonce_function_default, NULL) == 1);
    FUZZ_CHECK(secp256k1_fuzz_ecdsa_sha256_compression_calls != 0);
    FUZZ_CHECK(secp256k1_ecdsa_signature_serialize_compact(ctx, sig_explicit64, &explicit_sig) == 1);
    FUZZ_CHECK(memcmp(sig64, sig_explicit64, sizeof(sig64)) == 0);
    nonce_data.extra32 = NULL;
    ...
    secp256k1_fuzz_check_core_bip32_private_derivation(ctx, input, size);
    secp256k1_fuzz_check_core_ecdsa_sign_composition(ctx, input, size);
