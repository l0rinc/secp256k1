...
static void sha256_ellswift_zero_u(uint32_t *s, const unsigned char *msg, size_t rounds) {
    ...
    /* Zero the third compression block, which is the first u draw. */
    sha256_ellswift_zero_u_blocks += rounds;
    if (sha256_ellswift_zero_u_blocks == 3) {
...
void ellswift_zero_u_tests(void) {
    ...
    unsigned char ell64[64];
    secp256k1_pubkey expected, decoded;

    seckey[31] = 1;
    CHECK(secp256k1_ec_pubkey_create(ctx, &expected, seckey) == 1);
    sha256_ellswift_zero_u_blocks = 0;
